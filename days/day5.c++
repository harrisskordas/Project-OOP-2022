...
class Base{
    ...
    public:
        ...
        void sethealth(int );
        int gethealth();
        void getposition(int* ,int* ,Base* );    //x, y, target=creature
};
...
/*void Base::getposition(int* a, int* y, Base* cre){
    for(int i=0; i<x; i++){
        for(int j=0; j<y; j++){
            if (map[i][j] = cre)
        }
...
int main(int argc, char *argv[]){
    ...
    int wx, wy;
    int first_play, round=0, move_or_not, to_where;
    ...
    map.display();
    cout << "Every round ends when all creautures have been given the ability to move." << endl;
    ...
    if(first_play == 0){
        ...
        for(int i=0; i<vv.size(); i++){
            ...
            //vv[i]->getposition(&a,&b,vv[i]);



            move_or_not = rand()%(1);
            if(move_or_not == 0){
                cout << vv[i]->getname() << " decided to not move" << endl;
            }else{
                do{
                    to_where = rand()%(8);
                    if(to_where == 0){
                ...
                cout << vv[i]->getname() << " decided to move" << endl;
            }
        }
    }else{
